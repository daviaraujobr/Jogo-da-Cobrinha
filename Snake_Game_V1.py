...
class Snake_Game:
    def __init__(self):
        ...
        self.snake_position = [
            (41, 20), (41, 21), (41, 22), (41, 23), (42, 23), (43, 23)]
    ...
    def draw_map_elements(self, window):
        ...
        for y in range(self.map_size[1]):
            for x in range(self.map_size[0]):
                ...
                if self.map[y][x] == 's':
                    pg.draw.rect(
                        window, self.color['green'], (x * side, y * side, side, side))
                # Draw apple square position
                if self.map[y][x] == 'a':
                    pg.draw.rect(
                        window, self.color['red'], (x * side, y * side, side, side))

    def update_snake_position(self, snake):
        snake_size = len(snake) - 1
        for i in range(len(snake)):
            if snake_size - i == 0:
                self.snake_position[0] = (self.snake_position[0][0] + self.snake_direction[0],
                                          self.snake_position[0][1] + self.snake_direction[1])
            else:
                self.snake_position[snake_size -
                                    i] = self.snake_position[snake_size - i - 1]
    ...
    def snake_get_apple(self):
        ...
        if self.snake_position[0] == self.apple_position:
            self.snake_position.append(
                self.snake_position[len(self.snake_position) - 1])
    ...
    def draw_end_game(self, window):
        ...
        if self.end_game == True:
            score_text = self.font.render(
                'Fim de Jogo', True, self.color['white'])
...
while True:
    ...
    if w.home_menu == True:
        ...
        snake_game.add_snake_position(
            snake_game.map_size, snake_game.snake_position)
        snake_game.add_apple_position(
            snake_game.map_size, snake_game.apple_position)
